...
> Runner evidence (2026-08-23, v0.5.0 pipelines): the gate job caught three real defects local runs never showed (a bits-ui scroll-lock timer outliving jsdom teardown, a Vitest options-argument order that silently collected zero tests, and a Biome comment-formatting instability). The verify job timed out at 25m on a cold Chrome-for-Testing cache that GitLab then did not save (default cache:when on_success) — now 40m with when: always; it stays allow_failure until a warm-cache run proves it out. The release asset link must be built from the package job's dotenv-exported name: GitLab does not expand shell parameter operations in release YAML.
