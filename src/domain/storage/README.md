...
| Lives here | Why |
| --- | --- |
| `StorageErrorKind` (`Unavailable` / `NotFound` / `Conflict` / `Corrupt` / `Unexpected`) and the `StorageError` it tags | every repository port rejects with exactly this, so `chrome.runtime.lastError`, a quota `DOMException` and a malformed record all stop at the adapter boundary — the domain never sees them |
