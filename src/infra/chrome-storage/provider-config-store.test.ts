...
// store and the model-capability cache (card 83).
...
describe("ProviderDefaultsStore", () => {
  it("getBaseUrl is undefined until set", async () => {
    ...
    await expect(defaults.getBaseUrl("ollama")).resolves.toBeUndefined();
  });

  it("set/get round-trips, keyed by provider type", async () => {
    const { defaults } = setup();
    await defaults.setBaseUrl("ollama", "http://localhost:11434");
    await defaults.setBaseUrl("openai", "https://api.openai.com");

    await expect(defaults.getBaseUrl("ollama")).resolves.toBe("http://localhost:11434");
    await expect(defaults.getBaseUrl("openai")).resolves.toBe("https://api.openai.com");
  });
...
describe("ModelCapabilityCache", () => {
  it("get is undefined for a fingerprint never set", async () => {
    const { cache } = setup();
    await expect(cache.get("ollama", "digest-1")).resolves.toBeUndefined();
  });

  it("set/get round-trips, keyed by type + fingerprint", async () => {
    const { cache } = setup();
    await cache.set("ollama", "digest-1", { status: "tool-capable", detail: ["tools"] });
    await expect(cache.get("ollama", "digest-1")).resolves.toEqual({
      status: "tool-capable",
      detail: ["tools"],
    });
  });
  ...
  it("uses the byte-identical legacy key shape for ollama: 'ollama:cap:<digest>'", async () => {
    ...
    await cache.set("ollama", "digest-1", { status: "no-tools" });
  ...
  it("lives in local, not sync (derived, machine-specific data)", async () => {
    const { cache, fake } = setup();
    await cache.set("ollama", "digest-1", { status: "tool-capable" });
    expect(fake.sync.raw()).toEqual({});
  });
});
