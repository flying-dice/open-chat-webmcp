...
export function createChromeStorageProviderDefaultsStore(
  ...
  return {
    getBaseUrl: (type) =>
      readDecoded(local, baseUrlKey(type), (value) =>
...
export function createChromeStorageModelCapabilityCache(
  ...
  return {
    // A cache miss and an unreadable entry are the same outcome on purpose:
    // the caller re-asks the provider and re-files the answer, so a malformed
    // entry costs one round trip and then repairs itself. A storage FAILURE is
    // not folded in with them (card 92) — the caller recovers the same way,
    // but "the area did not answer" is worth a log line where "this entry is
    // stale" is not, which is exactly the split `readDecoded` keeps.
    get: (type, fingerprint) =>
      readDecoded(local, capabilityKey(type, fingerprint), (value) =>
        isRecord(value) &&
