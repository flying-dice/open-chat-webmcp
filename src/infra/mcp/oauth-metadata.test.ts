...
describe("discoverAuthorizationServer", () => {
  ...
  it("an RFC 9728 protected-resource document names a different authorization server issuer, and prefers ITS scopes_supported", async () => {
    stubFetchByUrl({
      "https://mcp.example/.well-known/oauth-protected-resource": () =>
        jsonResponse({ authorization_servers: ["https://other-as.example"], scopes_supported: ["resource-scope"] }),
      "https://other-as.example/.well-known/oauth-authorization-server": () => jsonResponse(AS_METADATA),
    });
  ...
  it("tries the path-inserted well-known URL first, falling back to the bare origin when it 404s (GitHub MCP server's shape)", async () => {
    ...
    expect(calledUrls.indexOf("https://mcp.example/.well-known/oauth-protected-resource/mcp")).toBeLessThan(
      calledUrls.indexOf("https://mcp.example/.well-known/oauth-protected-resource"),
    );
    // Path-inserted succeeded on the first try for the authorization-server metadata lookup.
    expect(calledUrls).toContain("https://github.com/.well-known/oauth-authorization-server/login/oauth");
  });
  ...
  it("RFC 8414 metadata missing token_endpoint is rejected as invalid-response", async () => {
    stubFetchByUrl({
      "https://mcp.example/.well-known/oauth-protected-resource": () => new Response("", { status: 404 }),
      "https://mcp.example/.well-known/oauth-authorization-server": () =>
        jsonResponse({ issuer: "https://mcp.example", authorization_endpoint: "https://mcp.example/authorize" }),
    });
    const result = await discoverAuthorizationServer("https://mcp.example");
  ...
  it("neither well-known location has RFC 8414 metadata at all: propagates the last (bare-origin) fetch failure", async () => {
    stubFetchByUrl({
      "https://mcp.example/.well-known/oauth-protected-resource": () => new Response("", { status: 404 }),
      "https://mcp.example/.well-known/oauth-authorization-server": () => new Response("", { status: 404, statusText: "Not Found" }),
    });
    const result = await discoverAuthorizationServer("https://mcp.example");
    expect(result.ok).toBe(false);
  });
...
describe("registerClient", () => {
  it("registers as a public client and returns the client id (+ optional secret)", async () => {
    const fetchMock = stubFetchByUrl({
      "https://as.example/register": () => jsonResponse({ client_id: "cid-1", client_secret: "shh" }),
    });

    const result = await registerClient("https://as.example/register", "https://redirect.example/cb");
  ...
  it("a non-2xx response is reported as invalid-response with the status and body", async () => {
    stubFetchByUrl({
      "https://as.example/register": () => new Response("invalid_client_metadata", { status: 400, statusText: "Bad Request" }),
    });
    const result = await registerClient("https://as.example/register", "https://redirect.example/cb");
    expect(result.ok).toBe(false);
    if (result.ok || result.error.kind !== "invalid-response") throw new Error("expected invalid-response");
