...
/** Optional auth for a server. Only a bearer token is supported for v1 — the same shape as a provider's `apiKey` (decisions/10), stored the same way. */
...
function isMcpServerAuth(v: unknown): v is McpServerAuth {
  return isRecord(v) && v.type === "bearer" && typeof v.token === "string" && v.token.length > 0;
}
