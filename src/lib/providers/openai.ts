...
//
// Never-throw discipline (decisions/09, carried from src/lib/ollama.ts):
...
function toOpenAiError(err: unknown): ProviderError {
  ...
  // way to distinguish them from here, mirroring src/lib/ollama.ts's
...
// bootstrap point) is what actually causes this side effect to run. Until
// something does, `createProviderClient` will throw for `type: "openai"`
// exactly as it does today for any unregistered type — a wiring gap for
// whoever adds the OpenAI option to the registry UI to close by importing
// this module, not a gap in the shared contract itself.
registerProviderType("openai", createOpenAiProvider);
